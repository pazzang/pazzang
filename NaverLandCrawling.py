...
area = {
    '서울시 강서구':'https://m.land.naver.com/cluster/ajax/articleList?rletTpCd=APT&tradTpCd=A1:B1:B2&cortarNo=1150000000',
    '서울시 구로구':'https://m.land.naver.com/cluster/ajax/articleList?rletTpCd=APT&tradTpCd=A1:B1:B2&cortarNo=1153000000',
    '서울시 노원구':'https://m.land.naver.com/cluster/ajax/articleList?rletTpCd=APT&tradTpCd=A1:B1:B2&cortarNo=1135000000',
    '서울시 중랑구':'https://m.land.naver.com/cluster/ajax/articleList?rletTpCd=APT&tradTpCd=A1:B1:B2&cortarNo=1126000000',
    '경기도 하남시':'https://m.land.naver.com/cluster/ajax/articleList?rletTpCd=APT&tradTpCd=A1:B1:B2&cortarNo=4145000000'}
...
def land_info(area_name,area_url):
    ...
    for i in tqdm.tqdm(range(1,301)):
        try:
    ...
    if info_df.empty:
        ...
        info_df = info_df.rename(columns = {'cortarNo':'지역코드','atclNo':'매물코드','vrfcTpCd':'매물정보','atclNm':'단지명','bildNm':'동',
...
#원하는 조건으로 정리
temp = total_df.copy()
temp['면적(평)'] = round(temp['면적']/3.3,2)
temp = temp.loc[(temp['매물방식'] == '매매')]
temp = temp.sort_values(by = ['거래가액','면적(평)'])
temp

temp.to_excel('20201119_매물정보_3.xlsx')
