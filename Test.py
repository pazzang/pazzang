...
# 거래가액 숫자로 치환
temp['가격(숫자)'] = temp['거래가액'].split("억")
if temp['가격(숫자)'][1] == "": 
    firstInt = int(temp['가격(숫자)'][0])*100000000
    secondInt = 0
    temp['가격(숫자)']  = firstInt + secondInt
else:
    firstInt = int(temp['가격(숫자)'][0])*100000000
    secondInt = int(temp['가격(숫자)'][1].replace(",", "")) *10000
    temp['가격(숫자)']  = firstInt + secondInt


temp = temp.loc[(temp['매물방식'] == '매매')&
                temp['가격(숫자)'] > 400000000 &  temp['가격(숫자)'] < 550000000]

temp

# Price
# 세안고, 승계 atclFetrDesc 특이사항


temp.to_excel('20201119_매물정보3.xlsx')
